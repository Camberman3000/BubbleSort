...
/* Init bools for type of sort */
bool bBubbleSortChosen = false;
bool bInsertionSortChosen = false;

void MoveCenter()// Moves console window to center screen
{
...
void InsertionSortList()
{
	...
	while (currentList != NULL)// Is current list valid?
	{
		...
		while (next != NULL && next->prev != NULL)// While next is NOT null and next's previous node is NOT null
		{
			...
			{
				break;// Break out back to the outer while loop
			}
		}
...
void MenuChooseFile()
{
	...
	if (result)
	{
		...
		if (bBubbleSortChosen == true)
		{
			BubbleSortList();// Sort the list
			bBubbleSortChosen = false;
		}
		else
		{
			InsertionSortList();
			bInsertionSortChosen = false;
		}	
...
int main()
{	 
	MoveCenter();// Moves console window to center screen (Upper left corner was annoying me - I have a large monitor)
	cout << "Mark's Sorting Program!" << endl;	
	...
	while (choice != 3)
	{
		...
		{
			bBubbleSortChosen = true;// Set bubble sort true
			MenuChooseFile();//
		}// end if choice == 1
		else if (choice == 2)// Insertion sort
		{
			bInsertionSortChosen = true;// set insertion sort true
			MenuChooseFile();			
		}
